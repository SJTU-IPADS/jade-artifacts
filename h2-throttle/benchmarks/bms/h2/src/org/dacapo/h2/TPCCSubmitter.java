...
public class TPCCSubmitter extends Submitter {
  ...
  public TPCCSubmitter(TPCCReporter reporter, Operations ops, OERandom rand, short maxW, int tid, int throttle) {
    super(null, ops, rand, maxW);
    this.rand = rand;
    this.scale = maxW;
    this.reporter = reporter;
  ...
  public long runTransactions(final Object displayData, final int count) throws Exception {
    if(this.throttle != 0) {
      return runTransactionsWithThrottle(displayData, count, this.throttle);
    }
  ...
  public long runTransactionsWithThrottle(final Object displayData, final int count, final int throttle) throws Exception {
    long targetOpsTickNs = (long) (1000000000 / throttle);
    // System.out.println(targetOpsTickNs);
    long startTime = System.nanoTime();
    long start = startTime;
    // long cur_time;
    long deadline = start;
    // retry = 0;
    // long deadline = startTime + targetOpsTickNs * tid / scale;
    for (int i = 0; i < count; i++) {
      ...
      int txType = getTransactionType();
      // while(currentThread.get() >= 3)Thread.sleep(1);
      // currentThread.getAndIncrement();
      // CompletableFuture<Void> cf = CompletableFuture.supplyAsync(() -> {
      // LatencyReporter.start(tid);
      LatencyReporter.startWithDeadline(tid, deadline);
      // }).thenAccept( txid -> {
      boolean success = false;
      while(!success) {
        try {
      //       // System.out.println("start " + txid);
          success = runTransaction(txType, displayData);  
        } catch (Exception e) {
      //       // this.retry++;
      ...
      //   }
      LatencyReporter.end(tid);
      //   opsDone.getAndIncrement();
      //   currentThread.getAndDecrement();
      // });
      transactionCount[txType]++;
      reporter.done();
      long end = System.nanoTime();
      
      deadline = deadline + targetOpsTickNs;
    }
    // while (opsDone.get() < count);
    // System.out.println(retry);
    System.out.printf("\nThroughput : %.1f op/s \n", (double) count/(((double)System.nanoTime() - (double)startTime)/1e9));
