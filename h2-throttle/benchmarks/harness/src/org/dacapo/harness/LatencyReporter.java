...
public class LatencyReporter {
  ...
  public static int startWithDeadline(int threadID, long deadline) {
    return reporters[threadID].startWithDeadline(deadline);
  }
  ...
  public int startWithDeadline(long deadline) {
    idx++;
    if (idx % stride == 0)
      idx = inc();
    if (idx < txbegin.length)
      startIdxWithDeadline(idx, deadline);
    return idx;
  }
  ...
  private static void startIdxWithDeadline(int index, long deadline) {
    if (callback != null)
      callback.requestStart(index);
    long start = (deadline - timerBase)/NS_COARSENING;
    txbegin[index] = (float) start;
    txend[index] = -1;
  }
  ...
  public static void reportLatency(String baseLatencyFileName, boolean dumpLatencyCSV, boolean dumpLatencyHDR, int iteration, int throttle) {
    if (timerBase != 0) {
